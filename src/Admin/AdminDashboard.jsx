...
const AdminDashboard = ({ onLogout, clientConfig }) => {
  ...
  const EMPTY_EXPENSE = { name: "", amount: "", currency: "USD", status: "unpaid", notes: "", category: "", dueDate: "", paidDate: "", vendorId: "" };
  const EMPTY_GIFT    = { from: "", description: "", amount: "", received: true, thankYouSent: false, notes: "" };

  // Helpers to get/set the right state for the active section
  const sectionData = { cyprus: expenses, lebanon: expLebanon, gifts };
  const sectionSetters = { cyprus: setExpenses, lebanon: setExpLebanon, gifts: setGifts };
  const activeExpenses = finSubTab === "gifts" ? [] : sectionData[finSubTab] || [];
  ...
  const handleExpSave = async () => {
    ...
    try {
      if (expForm.id) {
        const res = await axios.put(API_ENDPOINTS.UPDATE_FIN(finSubTab, expForm.id), expForm);
        sectionSetters[finSubTab]((prev) => prev.map((e) => e.id === expForm.id ? res.data : e));
      } else {
        const res = await axios.post(API_ENDPOINTS.CREATE_FIN(finSubTab), expForm);
        sectionSetters[finSubTab]((prev) => [...prev, res.data]);
      }
      setExpForm(null);
    } catch { setExpError("Failed to save expense."); }
  };

  const handleExpDelete = async () => {
    try {
      await axios.delete(API_ENDPOINTS.DELETE_FIN(finSubTab, deletingExpId));
      sectionSetters[finSubTab]((prev) => prev.filter((e) => e.id !== deletingExpId));
    } catch { setExpError("Failed to delete."); }
    finally { setDeletingExpId(null); }
  };

  const handleGiftSave = async () => {
    const resolvedFrom = giftForm.from === "__other__" ? (giftForm.fromCustom || "").trim() : giftForm.from;
    if (!resolvedFrom) return;
    const payload = { ...giftForm, from: resolvedFrom };
    delete payload.fromCustom;
    try {
      if (giftForm.id) {
        const res = await axios.put(API_ENDPOINTS.UPDATE_FIN("gifts", giftForm.id), payload);
        setGifts((prev) => prev.map((g) => g.id === giftForm.id ? res.data : g));
      } else {
        const res = await axios.post(API_ENDPOINTS.CREATE_FIN("gifts"), payload);
        setGifts((prev) => [...prev, res.data]);
      }
      setGiftForm(null);
    } catch { setExpError("Failed to save gift."); }
  };
  ...
  const handleGiftDelete = async () => {
    ...
    finally { setDeletingGiftId(null); }
  };

  // Export current section as .txt
  const handleExportTxt = () => {
    const data = finSubTab === "gifts" ? gifts : activeExpenses;
    const label = FIN_SECTIONS[finSubTab].label.toLowerCase().replace(/\s+/g, "-");
  ...
  const handleExportFinancialsExcel = () => {
    ...
    wsExp["!cols"] = [
      {wch:4},{wch:10},{wch:26},{wch:16},{wch:22},{wch:10},{wch:9},{wch:14},{wch:14},{wch:28},
    ];
    XLSX.utils.book_append_sheet(wb, wsExp, "Expenses");

    const wsGifts = XLSX.utils.json_to_sheet(giftRows);
    wsGifts["!cols"] = [{wch:4},{wch:22},{wch:20},{wch:10},{wch:10},{wch:14},{wch:28}];
    XLSX.utils.book_append_sheet(wb, wsGifts, "Gifts");

    XLSX.writeFile(wb, `financials-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const handleExportTablesExcel = () => {
    ...
    wsTables["!cols"] = [{wch:4},{wch:22},{wch:11},{wch:50},{wch:13},{wch:28}];
  ...
  const totalPaid     = activeExpenses.filter((e) => e.status === "paid").reduce((s, e) => s + (e.amount || 0), 0);
  ...
  const totalGifts    = gifts.filter((g) => g.amount).reduce((s, g) => s + (g.amount || 0), 0);
  ...
  const vendorTotals = (vendorId) => {
    ...
    const paid    = linked.filter((e) => e.status === "paid").reduce((s, e) => s + (e.amount || 0), 0);
    return { linked, total, paid, remaining: total - paid };
  };

  const fmt     = (n, cur)    => new Intl.NumberFormat((cur || finCurrency) === "EUR" ? "de-DE" : "en-US", { style: "currency", currency: cur || finCurrency }).format(n);
  ...
  const EMPTY_TABLE = { name: "", seats: "", shape: "round", priority: 3, notes: "", guests: [], position: null };

  const PRIORITY_COLORS = ["", "#c92a2a", "#e67700", "#2b8a3e", "#1971c2", "#868e96"];
  const PRIORITY_LABELS = ["", "P1 · VIP", "P2 · High", "P3 · Normal", "P4 · Low", "P5 · Filler"];
  ...
  const onFloorMove = (e) => {
    ...
    const rect = floorPlanRef.current.getBoundingClientRect();
    const x = Math.max(0, Math.min(e.clientX - rect.left - dragOffset.current.x, rect.width  - 120));
    const y = Math.max(0, Math.min(e.clientY - rect.top  - dragOffset.current.y, rect.height - 80));
    setTables((prev) => prev.map((t) => t.id === draggingId ? { ...t, position: { x, y } } : t));
  };

  const endDrag = async () => {
    if (!draggingId) return;
    const tbl = tables.find((t) => t.id === draggingId);
    if (tbl) await axios.put(API_ENDPOINTS.UPDATE_FIN("tables", tbl.id), tbl).catch(() => {});
    setDraggingId(null);
  };
  ...
  const EMPTY_TODO = { title: "", dueDate: "", assignee: "", status: "pending", notes: "" };
  ...
  const VENDOR_TYPES = ["Venue","Flowers","DJ / Music","Makeup","Photography","Videography","Catering","Cake","Dress / Attire","Transport","Decoration","Other"];
  const EMPTY_VENDOR = { name: "", type: "", status: "not-booked", contact: "", price: "", notes: "" };
  ...
  const saveSimple = async (section, form, setter, setForm) => {
    try {
      if (form.id) {
        const res = await axios.put(API_ENDPOINTS.UPDATE_FIN(section, form.id), form);
  ...
  const handleExportAttendingExcel = () => {
    ...
    XLSX.writeFile(wb, `attending-guests-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="admin-header">
        ...
        <button onClick={onLogout} className="admin-btn admin-btn-secondary admin-logout-btn">
          Logout
        </button>
      </div>
      ...
      {isRsvpSection && (
        <div className="admin-subnav">
          ...
          <button
            onClick={() => { setActiveTab("create"); setCreateResult(null); setCreateError(""); }}
          ...
          <div className="admin-card">
            ...
            <div className="create-gift-toggle">
              <label className="create-toggle-label">
                ...
                />
                Gift registry enabled for this family
              </label>
            </div>
            ...
            {createResult && (
              <div className="create-result success-message">
                ...
                <p>
                  ...
                  <code className="family-id-code">{createResult.family._id}</code>
                </p>
                <p>
          ...
          <div className="admin-subnav">
            {Object.entries(FIN_SECTIONS).map(([key, { label }]) => (
              <button
                ...
                onClick={() => { setFinSubTab(key); setExpError(""); }}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="admin-content">
            ...
            <div className="fin-overview">
              ...
              <div className="fin-overview-card fin-overview-grand">
                ...
                <span className="fin-overview-val">{fmt(totalCyprus + totalLebanon)}</span>
              </div>
              <div className="fin-overview-card fin-overview-gifts">
                <span className="fin-overview-label">Gifts Received</span>
                <span className="fin-overview-val">{gifts.length} ({fmt(totalGifts)})</span>
              </div>
              ...
              <div className="fin-overview-card fin-overview-export">
                <button
                  ...
                  disabled={expenses.length === 0 && expLebanon.length === 0 && gifts.length === 0}
              ...
              <>
                <div className="fin-summary">
                  <div className="fin-stat-card fin-stat-total">
                    <span className="fin-stat-amount">{fmt(totalExpenses)}</span>
                ...
                <div className="fin-toolbar">
                  ...
                  <div className="fin-toolbar-actions">
                    <button className="admin-btn admin-btn-ghost" onClick={() => fetchSection(finSubTab)} disabled={finLoading}>
                      ↺ Refresh
                    </button>
                    <button className="admin-btn admin-btn-ghost" onClick={handleExportTxt} disabled={activeExpenses.length === 0}>
                      Export .txt
                    </button>
                    <button className="admin-btn admin-btn-primary" onClick={() => setExpForm({ ...EMPTY_EXPENSE, currency: finCurrency })}>
                      + Add Expense
                    </button>
                  </div>
                </div>
                ...
                <div className="admin-card rsvp-table-card">
                  <div className="preview-table-container">
                    <table className="preview-table fin-table">
                      <thead>
                        <tr>
                          <th>#</th><th>Name</th><th>Category</th><th>Vendor</th>
                      ...
                      <tbody>
                        {activeExpenses.length === 0 ? (
                          <tr><td colSpan={9} className="rsvp-empty">No expenses yet. Click "+ Add Expense" to get started.</td></tr>
                        ) : (
                          activeExpenses.map((exp, i) => {
                            const linkedVendor = exp.vendorId ? vendors.find((v) => v.id === exp.vendorId) : null;
                            return (
                            <tr key={exp.id} className={`fin-row fin-row-${exp.status}`}>
                              <td className="rsvp-idx">{i + 1}</td>
                              <td className="fin-name">{exp.name}</td>
                              <td className="fin-category">{exp.category || "—"}</td>
                              <td className="fin-category">
                                {linkedVendor
                                  ? <span className="vendor-chip">{linkedVendor.name}</span>
                                  : <span className="fin-muted">—</span>}
                              </td>
                              <td className="fin-amount">{fmtExp(exp)}</td>
                              <td>
                                <span className={`rsvp-badge ${exp.status === "paid" ? "badge-yes" : "badge-no"}`}>
                                  {exp.status === "paid" ? "✓ Paid" : "✗ Unpaid"}
                                </span>
                              </td>
                              <td className="fin-date">
                                {exp.dueDate ? new Date(exp.dueDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                              </td>
                              <td className="fin-notes">{exp.notes || "—"}</td>
                              <td className="rsvp-actions">
                                <div className="rsvp-action-btns">
                                  <button className="admin-btn action-btn-edit" onClick={() => setExpForm({ id: exp.id, name: exp.name, amount: exp.amount, currency: exp.currency || finCurrency, status: exp.status, notes: exp.notes || "", category: exp.category || "", dueDate: exp.dueDate || "", paidDate: exp.paidDate || "", vendorId: exp.vendorId || "" })}>Edit</button>
                                  <button className="admin-btn action-btn-delete" onClick={() => setDeletingExpId(exp.id)}>Delete</button>
                                </div>
                              </td>
                            </tr>
                            );
                  ...
                  {activeExpenses.length > 0 && (
                    <p className="preview-note">
                      {activeExpenses.length} expense{activeExpenses.length !== 1 ? "s" : ""} · {activeExpenses.filter((e) => e.status === "paid").length} paid · {activeExpenses.filter((e) => e.status === "unpaid").length} unpaid
                    </p>
                  )}
                </div>
                )}
            ...
            {finSubTab === "gifts" && (
              <>
                <div className="fin-summary">
                  ...
                  <div className="fin-stat-card fin-stat-unpaid">
                    <span className="fin-stat-amount">{gifts.filter((g) => !g.thankYouSent).length}</span>
                ...
                <div className="fin-toolbar">
                  ...
                  <div className="fin-toolbar-actions">
                    <button className="admin-btn admin-btn-ghost" onClick={() => fetchSection("gifts")} disabled={finLoading}>↺ Refresh</button>
                    <button className="admin-btn admin-btn-ghost" onClick={handleExportTxt} disabled={gifts.length === 0}>Export .txt</button>
                    <button className="admin-btn admin-btn-primary" onClick={() => setGiftForm({ ...EMPTY_GIFT })}>+ Add Gift</button>
                  </div>
                </div>
                ...
                <div className="admin-card rsvp-table-card">
                  <div className="preview-table-container">
                    <table className="preview-table fin-table">
                      <thead>
                        <tr>
                          <th>#</th><th>From</th><th>Description</th>
                          <th>Amount</th><th>Received</th>
                          <th>Thank You</th><th>Notes</th><th>Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {gifts.length === 0 ? (
                          <tr><td colSpan={8} className="rsvp-empty">No gifts yet. Click "+ Add Gift" to get started.</td></tr>
                        ) : (
                          gifts.map((g, i) => (
                            <tr key={g.id} className="fin-row">
                              <td className="rsvp-idx">{i + 1}</td>
                              <td className="fin-name">{g.from}</td>
                              <td className="fin-category">{g.description || "—"}</td>
                              <td className="fin-amount">{g.amount ? fmt(g.amount) : "—"}</td>
                              <td>
                                <span className={`rsvp-badge ${g.received ? "badge-yes" : "badge-pending"}`}>
                                  {g.received ? "✓ Yes" : "— No"}
                                </span>
                              </td>
                              <td>
                                <span className={`rsvp-badge ${g.thankYouSent ? "badge-yes" : "badge-no"}`}>
                              ...
                              <td className="rsvp-actions">
                                <div className="rsvp-action-btns">
                                  <button className="admin-btn action-btn-edit" onClick={() => setGiftForm({ ...g, amount: g.amount ?? "" })}>Edit</button>
                                  <button className="admin-btn action-btn-delete" onClick={() => setDeletingGiftId(g.id)}>Delete</button>
                                </div>
                              </td>
                            </tr>
                          ))
                  ...
                  {gifts.length > 0 && (
                    <p className="preview-note">
                      {gifts.length} gift{gifts.length !== 1 ? "s" : ""} · {gifts.filter((g) => g.thankYouSent).length} thank yous sent · {gifts.filter((g) => !g.thankYouSent).length} pending
                    </p>
                  )}
                </div>
              </>
        ...
        <div className="modal-overlay" onClick={() => setExpForm(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close-btn" onClick={() => setExpForm(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="exp-form-grid">
                ...
                <div className="exp-form-field">
                  ...
                  <select
                    ...
                    onChange={(e) => setExpForm((f) => ({ ...f, vendorId: e.target.value }))}>
                ...
                <div className="exp-form-field">
                  ...
                  <input
                    ...
                    onChange={(e) => setExpForm((f) => ({ ...f, dueDate: e.target.value }))}
                  />
                </div>
                <div className="exp-form-field">
                  ...
                  <input
                    ...
                    onChange={(e) => setExpForm((f) => ({ ...f, paidDate: e.target.value }))}
                  />
                </div>
                <div className="exp-form-field exp-form-full">
              ...
              {(!expForm.name?.trim() || expForm.amount === "") && (
                <p className="exp-required-hint">* Name and amount are required</p>
              )}
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setExpForm(null)}>
                Cancel
              </button>
              <button
                className="admin-btn admin-btn-primary"
        ...
        <div className="modal-overlay" onClick={() => setDeletingExpId(null)}>
          <div className="modal-box modal-box-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close-btn" onClick={() => setDeletingExpId(null)}>✕</button>
            </div>
            <div className="modal-body">
              <p>Are you sure you want to delete this expense? <strong>This cannot be undone.</strong></p>
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setDeletingExpId(null)}>Cancel</button>
              <button className="admin-btn action-btn-delete" onClick={handleExpDelete}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setGiftForm(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close-btn" onClick={() => setGiftForm(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="exp-form-grid">
                <div className="exp-form-field exp-form-full">
                  ...
                  <select
                    className="admin-input"
                    value={giftForm.from}
                    onChange={(e) => setGiftForm((f) => ({ ...f, from: e.target.value }))}>
                    <option value="">— Select guest —</option>
                    {[...new Set(allRows.map((r) => r.name))].sort().map((name) => (
                  ...
                  {giftForm.from === "__other__" && (
                    <input
                      ...
                      onChange={(e) => setGiftForm((f) => ({ ...f, fromCustom: e.target.value }))}
                    />
                  )}
                </div>
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Description</label>
                  <input className="admin-input" placeholder="e.g. Cash, Crystal vase…" value={giftForm.description}
                    onChange={(e) => setGiftForm((f) => ({ ...f, description: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Amount (optional)</label>
                  <input className="admin-input" type="number" min="0" step="0.01" placeholder="0.00" value={giftForm.amount}
                    onChange={(e) => setGiftForm((f) => ({ ...f, amount: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Received?</label>
                  <select className="admin-input" value={giftForm.received ? "yes" : "no"}
                    onChange={(e) => setGiftForm((f) => ({ ...f, received: e.target.value === "yes" }))}>
                ...
                <div className="exp-form-field">
                  ...
                  <select className="admin-input" value={giftForm.thankYouSent ? "yes" : "no"}
                    onChange={(e) => setGiftForm((f) => ({ ...f, thankYouSent: e.target.value === "yes" }))}>
                ...
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Notes</label>
                  <textarea className="admin-input exp-notes-input" rows={2} placeholder="Any extra details…"
                    value={giftForm.notes} onChange={(e) => setGiftForm((f) => ({ ...f, notes: e.target.value }))} />
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setGiftForm(null)}>Cancel</button>
              <button className="admin-btn admin-btn-primary" onClick={handleGiftSave}
                disabled={!giftForm?.from || giftForm.from === "__other__" ? !(giftForm?.fromCustom?.trim()) : false}>
      ...
      {deletingGiftId && (
        <div className="modal-overlay" onClick={() => setDeletingGiftId(null)}>
          <div className="modal-box modal-box-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Delete Gift</h2>
              <button className="modal-close-btn" onClick={() => setDeletingGiftId(null)}>✕</button>
            </div>
            <div className="modal-body">
              <p>Are you sure you want to delete this gift record? <strong>This cannot be undone.</strong></p>
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setDeletingGiftId(null)}>Cancel</button>
              <button className="admin-btn action-btn-delete" onClick={handleGiftDelete}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
              ...
              {editAttendees.map((att, i) => (
                <div key={i} className="modal-attendee-row">
                  <input
                    ...
                    value={att.name}
                    onChange={(e) =>
                      handleEditAttendeeName(i, e.target.value)
                    }
                  />
                  <select
      ...
      {deletingFamilyId && (
        <div
          ...
          <div
            ...
            <div className="modal-body">
              <p>
                Are you sure you want to delete this family and all their
                guest records? <strong>This cannot be undone.</strong>
              </p>
              <p className="modal-family-id">
        ...
        <div
          ...
          <div
            ...
            <div className="modal-body">
              ...
              {/* Photo section */}
              <div className="invite-section-header" style={{ marginTop: "1rem" }}>
              ...
              {invitePhoto ? (
                ...
                <label className="invite-photo-upload-label">
                  <input
                    ...
                    onChange={(e) =>
                      setInvitePhoto(e.target.files[0] || null)
                    }
                  />
                  + Upload Photo
                </label>
        ...
        <div className="admin-content">
          <div className="fin-toolbar">
            ...
            <div className="fin-toolbar-actions">
              <div className="view-toggle-group">
                <button className={`view-toggle-btn ${!floorPlanMode ? "active" : ""}`} onClick={() => setFloorPlanMode(false)}>List</button>
                <button className={`view-toggle-btn ${floorPlanMode  ? "active" : ""}`} onClick={() => setFloorPlanMode(true)}>Floor Plan</button>
              </div>
              <button className="admin-btn admin-btn-ghost" onClick={() => fetchSimple("tables", setTables)}>↺ Refresh</button>
              <button className="admin-btn admin-btn-primary" onClick={() => setTableForm({ ...EMPTY_TABLE })}>+ Add Table</button>
            </div>
          </div>
          ...
          {!floorPlanMode && (
            <div className="tables-grid">
              {tables.length === 0 && (
                <div className="rsvp-empty" style={{ padding: "2rem", textAlign: "center" }}>No tables yet. Click "+ Add Table" to start.</div>
              )}
              {[...tables].sort((a, b) => (a.priority || 3) - (b.priority || 3)).map((tbl) => (
                <div key={tbl.id} className="table-card">
                  <div className="table-card-header">
                    <span className="table-card-name">{tbl.name}</span>
                    <div className="table-card-meta">
                      {tbl.priority && (
                        <span className="table-priority-badge" style={{ background: PRIORITY_COLORS[tbl.priority] + "22", color: PRIORITY_COLORS[tbl.priority], borderColor: PRIORITY_COLORS[tbl.priority] + "66" }}>
                          {PRIORITY_LABELS[tbl.priority]}
                        </span>
                      )}
                      {tbl.shape && (
                        <span className={`table-shape-badge shape-badge-${tbl.shape}`}>
                          <span className={`table-shape-icon shape-${tbl.shape} shape-icon-sm`} />
                          {tbl.shape.charAt(0).toUpperCase() + tbl.shape.slice(1)}
                        </span>
                      )}
                      <span className="table-card-seats">{(tbl.guests || []).length}{tbl.seats ? `/${tbl.seats}` : ""} seats</span>
                    </div>
                  </div>
                  {tbl.guests && tbl.guests.length > 0 && (
                    <ul className="table-guest-list">
                      {tbl.guests.map((g, i) => <li key={i}>{g}</li>)}
                    </ul>
                  )}
                  {tbl.notes && <p className="table-notes">{tbl.notes}</p>}
                  <div className="table-card-actions">
                    <button className="admin-btn action-btn-edit" onClick={() => setTableForm({ ...tbl, guests: [...(tbl.guests || [])] })}>Edit</button>
                    <button className="admin-btn action-btn-delete" onClick={() => setDeletingTableId(tbl.id)}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {floorPlanMode && (
            <div
              ...
              {tables.length === 0 && (
                <div className="fp-empty">Add tables first, then arrange them here.</div>
              )}
              {tables.map((tbl, i) => {
                const pos    = tbl.position || { x: 20 + (i % 5) * 190, y: 20 + Math.floor(i / 5) * 160 };
                const pColor = PRIORITY_COLORS[tbl.priority || 3];
                return (
                  <div
                    key={tbl.id}
                    className={`fp-table fp-shape-${tbl.shape || "round"} ${draggingId === tbl.id ? "fp-dragging" : ""}`}
                    style={{ left: pos.x, top: pos.y, borderColor: pColor, "--fp-color": pColor }}
                    onMouseDown={(e) => startDrag(e, tbl)}>
                    <span className="fp-priority" style={{ background: pColor }}>{tbl.priority || 3}</span>
                    <span className="fp-table-name">{tbl.name}</span>
                    <span className="fp-table-count">{(tbl.guests || []).length}{tbl.seats ? `/${tbl.seats}` : ""}</span>
                    {(tbl.guests || []).slice(0, 4).map((g, gi) => (
                      <span key={gi} className="fp-guest-dot" title={g}>{g.split(" ")[0]}</span>
                    ))}
                    {(tbl.guests || []).length > 4 && (
                      <span className="fp-guest-dot fp-more">+{tbl.guests.length - 4}</span>
                    )}
                  </div>
                );
              })}
        ...
        <div className="modal-overlay" onClick={() => setTableForm(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close-btn" onClick={() => setTableForm(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="exp-form-grid">
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Table Name / Number *</label>
                  <input className="admin-input" placeholder="e.g. Table 1, VIP Table…" value={tableForm.name}
                    onChange={(e) => setTableForm((f) => ({ ...f, name: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Max Seats</label>
                  <input className="admin-input" type="number" min="1" placeholder="10" value={tableForm.seats}
                  ...
                  <div className="table-shape-picker">
                    {[
                      ...
                      <button
                        ...
                        onClick={() => setTableForm((f) => ({ ...f, shape: value }))}>
                ...
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Priority <span style={{ fontWeight: 400, color: "#868e96" }}>(1 = VIP / most important, 5 = filler)</span></label>
                  <div className="priority-picker">
                    {[1, 2, 3, 4, 5].map((p) => (
                      <button
                        ...
                        onClick={() => setTableForm((f) => ({ ...f, priority: p }))}>
                        {p}
                        <span className="priority-btn-label">{["", "VIP", "High", "Normal", "Low", "Filler"][p]}</span>
                      </button>
                    ))}
                  </div>
                </div>
                <div className="exp-form-field exp-form-full">
                  ...
                  <select className="admin-input" onChange={(e) => {
                    ...
                    if (name && !tableForm.guests.includes(name))
                      setTableForm((f) => ({ ...f, guests: [...f.guests, name] }));
                    e.target.value = "";
                  }}>
                    <option value="">— Add a guest —</option>
                    {[...new Set(allRows.map((r) => r.name))].sort()
                  ...
                  <div className="table-assigned-tags">
                    {tableForm.guests.map((g) => (
                      <span key={g} className="table-guest-tag">
                        {g}
                        <button onClick={() => setTableForm((f) => ({ ...f, guests: f.guests.filter((x) => x !== g) }))}>✕</button>
                      </span>
                    ))}
                  </div>
                </div>
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Notes</label>
                  <textarea className="admin-input exp-notes-input" rows={2} placeholder="e.g. Near stage, vegetarian side…"
                    value={tableForm.notes} onChange={(e) => setTableForm((f) => ({ ...f, notes: e.target.value }))} />
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setTableForm(null)}>Cancel</button>
              <button className="admin-btn admin-btn-primary" disabled={!tableForm.name?.trim()}
                onClick={() => saveSimple("tables", tableForm, setTables, setTableForm)}>
      ...
      {deletingTableId && (
        <div className="modal-overlay" onClick={() => setDeletingTableId(null)}>
          <div className="modal-box modal-box-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>Delete Table</h2><button className="modal-close-btn" onClick={() => setDeletingTableId(null)}>✕</button></div>
            <div className="modal-body"><p>Delete this table? <strong>Cannot be undone.</strong></p></div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setDeletingTableId(null)}>Cancel</button>
              <button className="admin-btn action-btn-delete" onClick={() => deleteSimple("tables", deletingTableId, setTables, setDeletingTableId)}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === "todos" && (
        <div className="admin-content">
          <div className="fin-toolbar">
            ...
            <div className="fin-toolbar-actions">
              <button className="admin-btn admin-btn-ghost" onClick={() => fetchSimple("todos", setTodos)}>↺ Refresh</button>
              <button className="admin-btn admin-btn-primary" onClick={() => setTodoForm({ ...EMPTY_TODO })}>+ Add Task</button>
            </div>
          </div>
          ...
          {todos.length > 0 && (() => {
            ...
            return (
              <div className="todo-progress-bar-wrap">
                ...
                <span className="todo-progress-label">{done}/{todos.length} completed ({pct}%)</span>
              </div>
            );
          })()}
          ...
          <div className="admin-card rsvp-table-card">
            <div className="preview-table-container">
              <table className="preview-table fin-table">
                <thead>
                  <tr><th>#</th><th>Task</th><th>Due Date</th><th>Assignee</th><th>Status</th><th>Notes</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  {todos.length === 0 ? (
                    <tr><td colSpan={7} className="rsvp-empty">No tasks yet. Click "+ Add Task" to get started.</td></tr>
                  ) : (
                    todos.map((t, i) => (
                      <tr key={t.id} className={`fin-row ${t.status === "done" ? "todo-row-done" : t.status === "in-progress" ? "todo-row-wip" : ""}`}>
                        <td className="rsvp-idx">{i + 1}</td>
                        <td className="fin-name" style={{ textDecoration: t.status === "done" ? "line-through" : "none" }}>{t.title}</td>
                        <td className="fin-date">{t.dueDate ? new Date(t.dueDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}</td>
                        <td>{t.assignee || "—"}</td>
                        <td>
                          <span className={`rsvp-badge ${t.status === "done" ? "badge-yes" : t.status === "in-progress" ? "badge-pending" : "badge-no"}`}>
                            {t.status === "done" ? "✓ Done" : t.status === "in-progress" ? "⏳ In Progress" : "◦ Pending"}
                          </span>
                        </td>
                        <td className="fin-notes">{t.notes || "—"}</td>
                        <td className="rsvp-actions">
                          <div className="rsvp-action-btns">
                            <button className="admin-btn action-btn-edit" onClick={() => setTodoForm({ ...t })}>Edit</button>
                            <button className="admin-btn action-btn-delete" onClick={() => setDeletingTodoId(t.id)}>Delete</button>
                          </div>
                        </td>
                      </tr>
                    ))
        ...
        <div className="modal-overlay" onClick={() => setTodoForm(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close-btn" onClick={() => setTodoForm(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="exp-form-grid">
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Task *</label>
                  <input className="admin-input" placeholder="e.g. Book florist, Send invitations…" value={todoForm.title}
                    onChange={(e) => setTodoForm((f) => ({ ...f, title: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Due Date</label>
                  <input className="admin-input" type="date" value={todoForm.dueDate}
                    onChange={(e) => setTodoForm((f) => ({ ...f, dueDate: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Assignee</label>
                  <input className="admin-input" placeholder="e.g. Justin, Yara…" value={todoForm.assignee}
                    onChange={(e) => setTodoForm((f) => ({ ...f, assignee: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Status</label>
                  <select className="admin-input" value={todoForm.status}
                    onChange={(e) => setTodoForm((f) => ({ ...f, status: e.target.value }))}>
                ...
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Notes</label>
                  <textarea className="admin-input exp-notes-input" rows={2} value={todoForm.notes}
                    onChange={(e) => setTodoForm((f) => ({ ...f, notes: e.target.value }))} />
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setTodoForm(null)}>Cancel</button>
              <button className="admin-btn admin-btn-primary" disabled={!todoForm.title?.trim()}
                onClick={() => saveSimple("todos", todoForm, setTodos, setTodoForm)}>
      ...
      {deletingTodoId && (
        <div className="modal-overlay" onClick={() => setDeletingTodoId(null)}>
          <div className="modal-box modal-box-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>Delete Task</h2><button className="modal-close-btn" onClick={() => setDeletingTodoId(null)}>✕</button></div>
            <div className="modal-body"><p>Delete this task? <strong>Cannot be undone.</strong></p></div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setDeletingTodoId(null)}>Cancel</button>
              <button className="admin-btn action-btn-delete" onClick={() => deleteSimple("todos", deletingTodoId, setTodos, setDeletingTodoId)}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === "vendors" && (
        <div className="admin-content">
          <div className="fin-toolbar">
            ...
            <div className="fin-toolbar-actions">
              <button className="admin-btn admin-btn-ghost" onClick={() => fetchSimple("vendors", setVendors)}>↺ Refresh</button>
              <button className="admin-btn admin-btn-primary" onClick={() => setVendorForm({ ...EMPTY_VENDOR })}>+ Add Vendor</button>
            </div>
          </div>
          ...
          {vendors.length > 0 && (
            <div className="fin-summary">
              ...
              <div className="fin-stat-card fin-stat-paid">
                <span className="fin-stat-amount">{vendors.filter((v) => v.status === "booked").length}</span>
                <span className="fin-stat-label">Booked</span>
              </div>
              <div className="fin-stat-card fin-stat-unpaid">
                <span className="fin-stat-amount">{vendors.filter((v) => v.status === "not-booked").length}</span>
          ...
          <div className="admin-card rsvp-table-card">
            <div className="preview-table-container">
              <table className="preview-table fin-table">
                <thead>
                  <tr><th>#</th><th>Name</th><th>Type</th><th>Status</th><th>Contact</th><th>Quoted Price</th><th>Paid</th><th>Remaining</th><th>Notes</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  {vendors.length === 0 ? (
                    <tr><td colSpan={10} className="rsvp-empty">No vendors yet. Click "+ Add Vendor".</td></tr>
                  ) : (
                    vendors.map((v, i) => {
                      ...
                      return (
                        <>
                          <tr key={v.id} className={`fin-row ${v.status === "booked" ? "fin-row-paid" : "fin-row-unpaid"}`}>
                            <td className="rsvp-idx">{i + 1}</td>
                            <td className="fin-name">
                              <button
                                className="vendor-expand-btn"
                                onClick={() => setExpandedVendorId(isExpanded ? null : v.id)}
                                title={isExpanded ? "Collapse" : "Show linked expenses"}>
                                {isExpanded ? "▾" : "▸"}
                              </button>
                              {v.name}
                              {vt.linked.length > 0 && (
                                <span className="vendor-exp-count">{vt.linked.length} exp.</span>
                              )}
                            </td>
                            <td className="fin-category">{v.type || "—"}</td>
                            <td>
                              <span className={`rsvp-badge ${v.status === "booked" ? "badge-yes" : "badge-no"}`}>
                                {v.status === "booked" ? "✓ Booked" : "✗ Not Booked"}
                              </span>
                            </td>
                            <td className="fin-category">{v.contact || "—"}</td>
                            <td className="fin-amount">{v.price ? fmt(parseFloat(v.price)) : "—"}</td>
                            <td className="fin-amount fin-paid-cell">
                              {vt.linked.length > 0 ? <span className="vendor-paid">{fmt(vt.paid)}</span> : "—"}
                            </td>
                            <td className="fin-amount fin-unpaid-cell">
                              {vt.linked.length > 0
                                ? <span className={`vendor-remaining ${vt.remaining > 0 ? "has-remaining" : "fully-paid"}`}>
                                    {fmt(vt.remaining)}
                                  </span>
                                : "—"}
                            </td>
                            ...
                            <td className="rsvp-actions">
                              <div className="rsvp-action-btns">
                                <button className="admin-btn action-btn-edit" onClick={() => setVendorForm({ ...v })}>Edit</button>
                                <button className="admin-btn action-btn-delete" onClick={() => setDeletingVendorId(v.id)}>Delete</button>
                              </div>
                            </td>
                          </tr>
                          {isExpanded && (
                            <tr key={`${v.id}-detail`} className="vendor-detail-row">
                              <td colSpan={10}>
                                {vt.linked.length === 0 ? (
                                  <p className="vendor-detail-empty">No expenses linked to this vendor yet. Link them from the Financials tab.</p>
                                ) : (
                                  <div className="vendor-detail-wrap">
                                    <div className="vendor-detail-bar">
                                      <span className="vd-label">Total linked</span>
                                      <span className="vd-val">{fmt(vt.total)}</span>
                                      <span className="vd-sep"/>
                                      <span className="vd-label vd-paid">Paid</span>
                                      <span className="vd-val vd-paid">{fmt(vt.paid)}</span>
                                      <span className="vd-sep"/>
                                      <span className="vd-label vd-rem">Remaining</span>
                                      <span className="vd-val vd-rem">{fmt(vt.remaining)}</span>
                                    </div>
                                    <div className="vendor-progress-wrap">
                                      <div
                                        className="vendor-progress-fill"
                                        style={{ width: vt.total > 0 ? `${(vt.paid / vt.total) * 100}%` : "0%" }}
                                      />
                                    </div>
                                    <table className="vendor-linked-table">
                                      <thead>
                                        <tr><th>Expense</th><th>Wedding</th><th>Amount</th><th>Status</th></tr>
                                      </thead>
                                      <tbody>
                                        {vt.linked.map((e) => (
                                          <tr key={e.id}>
                                            <td>{e.name}</td>
                                            <td className="fin-category">{expenses.find((x) => x.id === e.id) ? "Cyprus" : "Lebanon"}</td>
                                            <td className="fin-amount">{fmt(e.amount)}</td>
                                            <td>
                                              <span className={`rsvp-badge ${e.status === "paid" ? "badge-yes" : "badge-no"}`}>
        ...
        <div className="modal-overlay" onClick={() => setVendorForm(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close-btn" onClick={() => setVendorForm(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="exp-form-grid">
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Vendor Name *</label>
                  <input className="admin-input" placeholder="e.g. Rose Garden Florist" value={vendorForm.name}
                    onChange={(e) => setVendorForm((f) => ({ ...f, name: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Type</label>
                  <select className="admin-input" value={vendorForm.type}
                ...
                <div className="exp-form-field">
                  <label className="exp-label">Status</label>
                  <select className="admin-input" value={vendorForm.status}
                ...
                <div className="exp-form-field">
                  ...
                  <input className="admin-input" placeholder="Phone / email…" value={vendorForm.contact}
                    onChange={(e) => setVendorForm((f) => ({ ...f, contact: e.target.value }))} />
                </div>
                <div className="exp-form-field">
                  <label className="exp-label">Price</label>
                  <input className="admin-input" type="number" min="0" step="0.01" placeholder="0.00" value={vendorForm.price}
                    onChange={(e) => setVendorForm((f) => ({ ...f, price: e.target.value }))} />
                </div>
                <div className="exp-form-field exp-form-full">
                  <label className="exp-label">Notes</label>
                  <textarea className="admin-input exp-notes-input" rows={2} value={vendorForm.notes}
                    onChange={(e) => setVendorForm((f) => ({ ...f, notes: e.target.value }))} />
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setVendorForm(null)}>Cancel</button>
              <button className="admin-btn admin-btn-primary" disabled={!vendorForm.name?.trim()}
                onClick={() => saveSimple("vendors", vendorForm, setVendors, setVendorForm)}>
      ...
      {deletingVendorId && (
        <div className="modal-overlay" onClick={() => setDeletingVendorId(null)}>
          <div className="modal-box modal-box-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>Delete Vendor</h2><button className="modal-close-btn" onClick={() => setDeletingVendorId(null)}>✕</button></div>
            <div className="modal-body"><p>Delete this vendor? <strong>Cannot be undone.</strong></p></div>
            <div className="modal-footer">
              <button className="admin-btn admin-btn-ghost" onClick={() => setDeletingVendorId(null)}>Cancel</button>
              <button className="admin-btn action-btn-delete" onClick={() => deleteSimple("vendors", deletingVendorId, setVendors, setDeletingVendorId)}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
